...
class ManifestStorage(ManifestStaticFilesStorage):
    ...
    def _initial(self, extension):
        return [
            f"{self.prefix}{key}"
            for key, value in self.manifest.items()
            if value["isInitial"] and value["path"].endswith(extension)
        ]

    def initial_scripts(self):
        return self._initial(".js")

    def initial_stylesheets(self):
        return self._initial(".css")
